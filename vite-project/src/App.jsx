...
// 2. Create a Layout for the main user-facing application
const UserLayout = () => {
  const location = useLocation();
  const dispatch = useDispatch();
  const navigate = useNavigate();
      ...
      <Box sx={{
        ...
        <Container maxWidth="lg" sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          >
            ...
            <Tab label="Know Hub" value="/know-hub" to="/know-hub" component={Link} />
            <Tab label="Profile" value="/profile" to="/profile" component={Link} />
            {/* Admin link is now part of the main nav, but only for admins */}
...
// 3. Create a component to protect admin-only routes
const AdminRoute = () => {
  const { user } = useSelector((state) => state.auth);
  // If user is an admin, show the AdminLayout. Otherwise, redirect.
  return user && user.user.role === 'Admin' ? <AdminLayout /> : <Navigate to="/" />;
...
// 4. The main App component is now much simpler
function App() {
  const { user } = useSelector((state) => state.auth);
  ...
  return (
    <Routes>
      ...
      <Route path="/admin" element={<AdminRoute />}>
        <Route path="articles" element={<ArticleListPage />} />
        <Route path="articles/create" element={<CreateArticlePage />} />
        {/* 2. Add the new, dynamic route for editing an article */}
        <Route path="articles/edit/:id" element={<EditArticlePage />} />
        {/* We can add a default dashboard route here */}
